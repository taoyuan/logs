...
Library.prototype.get = function (name, color, category, colors, options) {
  ...
  var logger = new Logger(this.provider.getLogger((category.join(' ')), options));
  var that = this;
   ...
   */
  logger.extend = function (name, color) {
    return that.get(name, color, category, colors, options);
  };
...
function get(name, color, category, colors, options) {
  ...
  return global[LIBRARY].get(name, color, category, colors, options);
}
